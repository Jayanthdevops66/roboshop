...
if [ $? -ne 0 ]; then
  ...
  echo "alter user 'root'@'localhost' identified with mysql_native_password by '$MYSQL_PASSWORD';" | mysql --connect-expired-password -u root -p${DEFAULT_PASSWORD}
  StatusCheck
fi

echo "show pluggins;" | mysql -uroot -p$MYSQL_PASSWORD 2>&1 | grep vallidate_password &>>${LOG}
if [ $? -eq 0 ]; then
